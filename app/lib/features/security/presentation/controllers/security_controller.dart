...
final class SecurityState {
  ...
  SecurityState copyWith({
    ...
    NativeSecurityStatus? nativeStatus,
    bool? isLocked,
  }) => SecurityState(
...
class SecurityController extends AsyncNotifier<SecurityState> {
  ...
  Future<bool> configurePin(String pin) async {
    ...
    state = AsyncData(
      current.copyWith(preferences: preferences, isLocked: false),
    );
    return true;
  }
  ...
  Future<bool> changePin(String currentPin, String newPin) async {
    ...
    await (await ref.read(
      securityRepositoryProvider.future,
    )).recordAudit('pin_changed', success: true);
    return true;
  }
  ...
  Future<void> updatePreferences(SecurityPreferences preferences) async {
    final current = state.requireValue;
    final safe = preferences.copyWith(
      pinEnabled: current.preferences.pinEnabled,
    );
    await (await ref.read(
      securityRepositoryProvider.future,
    )).savePreferences(safe);
