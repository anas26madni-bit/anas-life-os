...
final class DriftSecurityRepository implements SecurityRepository {
  ...
  Future<void> savePreferences(SecurityPreferences preferences) async {
    ...
    await _database
        .into(_database.securitySettings)
  ...
  Future<void> recordUnlock({
    ...
    final now = _clock().toUtc().microsecondsSinceEpoch;
    await _database
        .into(_database.appLockSessions)
        .insert(
          AppLockSessionsCompanion.insert(
            uuid: _uuidFactory(),
            sessionStart: now,
            sessionEnd: Value(success ? now : null),
            unlockMethod: method.name,
            success: success,
  ...
  Future<void> recordAudit(String action, {required bool success}) async {
    await _database
        .into(_database.securityAuditLog)
        .insert(
