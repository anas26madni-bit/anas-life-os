...
@TableIndex(name: 'idx_knowledge_folders_space_parent', columns: {#spaceId, #parentFolderId})
class KnowledgeFolders extends BusinessEntityTable {
  IntColumn get spaceId => integer().references(KnowledgeSpaces, #id)();
  IntColumn get parentFolderId => integer().nullable().references(KnowledgeFolders, #id)();
  TextColumn get name => text().withLength(min: 1, max: 200)();
  TextColumn get description => text().nullable()();
  TextColumn get icon => text().nullable()();
  TextColumn get color => text().nullable()();
...
@TableIndex(name: 'idx_knowledge_notes_space_updated', columns: {#spaceId, #updatedAt})
@TableIndex(name: 'idx_knowledge_notes_folder_updated', columns: {#folderId, #updatedAt})
@TableIndex(name: 'idx_knowledge_notes_type_status', columns: {#noteType, #status})
@TableIndex(name: 'idx_knowledge_notes_title', columns: {#title})
class KnowledgeNotes extends BusinessEntityTable {
  IntColumn get spaceId => integer().references(KnowledgeSpaces, #id)();
  IntColumn get folderId => integer().nullable().references(KnowledgeFolders, #id)();
...
@TableIndex(name: 'idx_knowledge_note_tags_pair', columns: {#noteId, #tagId}, unique: true)
class KnowledgeNoteTags extends Table {
  IntColumn get id => integer().autoIncrement()();
  IntColumn get noteId => integer().references(KnowledgeNotes, #id, onDelete: KeyAction.cascade)();
  IntColumn get tagId => integer().references(KnowledgeTags, #id, onDelete: KeyAction.cascade)();
}

@DataClassName('KnowledgeLinkRow')
@TableIndex(name: 'idx_knowledge_links_pair', columns: {#sourceNoteId, #targetNoteId, #linkType}, unique: true)
class KnowledgeLinks extends Table {
  IntColumn get id => integer().autoIncrement()();
  TextColumn get uuid => text().withLength(min: 36, max: 36).unique()();
  IntColumn get sourceNoteId => integer().references(KnowledgeNotes, #id, onDelete: KeyAction.cascade)();
  IntColumn get targetNoteId => integer().references(KnowledgeNotes, #id, onDelete: KeyAction.cascade)();
  TextColumn get linkType => textEnum<KnowledgeLinkType>()();
  IntColumn get createdAt => integer()();
}

@DataClassName('KnowledgeVersionRow')
@TableIndex(name: 'idx_knowledge_versions_note_version', columns: {#noteId, #versionNumber}, unique: true)
