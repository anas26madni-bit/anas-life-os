...
final class DriftStatisticsRepository implements StatisticsRepository {
  ...
  Future<void> _rebuildDay(DateTime localDay, int? projectId) async {
    ...
    await _database
        .into(_database.dailyStatisticsProjections)
        .insertOnConflictUpdate(
          DailyStatisticsProjectionsCompanion.insert(
            scopeKey: _scope(projectId),
            projectId: Value(projectId),
            periodStartUtc: startMicros,
            periodEndUtc: endMicros,
            timezoneOffsetMinutes: start.timeZoneOffset.inMinutes,
            eligibleCount: eligible,
            completedByEndCount: completed,
            onTimeCount: onTime,
            delayTotalMicroseconds: delayTotal,
            delaySampleCount: delayRows.length,
            rebuiltAt: _clock().toUtc().microsecondsSinceEpoch,
          ),
        );
  }
