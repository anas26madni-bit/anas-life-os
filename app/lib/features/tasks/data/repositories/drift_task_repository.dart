...
final class DriftTaskRepository implements TaskRepository {
  ...
  Future<Result<TaskEntity>> changeStatus(int id, TaskStatus status, {String? reason}) async {
    ...
    try {
      return await _database.transaction(() async {
        final current = await _support.requireRaw(id, includeDeleted: status == TaskStatus.deleted);
        if (status == TaskStatus.completed) await _support.validateMandatoryChildren(id);
  ...
  Future<Result<TaskEntity>> restore(int id, {String? reason}) async {
    try {
      ...
      if (current.parentTaskId != null) await _support.requireRaw(current.parentTaskId!, includeDeleted: false);
  ...
  Future<Result<TaskEntity>> _restoreTo(int id, TaskStatus restored, String? reason) async {
    return _database.transaction(() async {
      ...
      await _support.recordState(id, TaskStatus.deleted, restored, now, reason ?? 'restored');
      await _support.refreshParentCount(current.parentTaskId, now);
      return Success(await _requireRow(id));
    });
  }
  ...
  Future<Result<void>> addDependency({required int taskId, required int dependsOnTaskId, required DependencyType type}) async {
    ...
    try {
      return await _database.transaction(() async {
        ...
        final duplicate = await (_database.select(_database.taskDependencies)
              ..where(
                (row) =>
  ...
  Future<TaskEntity> _requireRow(
    int id, {
    bool includeDeleted = false,
  }) async => _map(
    await _support.requireRaw(id, includeDeleted: includeDeleted),
  );
