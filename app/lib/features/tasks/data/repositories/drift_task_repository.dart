...
final class DriftTaskRepository implements TaskRepository {
  ...
  Future<Result<TaskEntity>> update(int id, TaskDraft draft) async {
    ...
    try {
      return await _database.transaction(() async {
        ...
        final now = _clock().toUtc();
        await (_database.update(_database.tasks)..where((row) => row.id.equals(id))).write(
          TasksCompanion(
            ...
            startAt: Value(_epoch(normalized.startAt)),
            dueAt: Value(_epoch(normalized.dueAt)),
            completedAt: Value(normalized.status == TaskStatus.completed ? now.microsecondsSinceEpoch : null),
            estimatedMinutes: Value(normalized.estimatedMinutes),
            energyLevel: Value(normalized.energyLevel),
            difficulty: Value(normalized.difficulty),
            color: Value(normalized.color),
        ...
        if (current.status != normalized.status) {
          await _recordState(id, current.status, normalized.status, now, 'updated');
        }
      ...
      return FailureResult(
        ValidationFailure(code: 'task_update_rejected', safeMessage: error.message.toString()),
      );
    } on Object {
      return const FailureResult(
        DatabaseFailure(code: 'task_update_failed', safeMessage: 'The task could not be updated safely.'),
      );
    }
  }

  @override
  Future<Result<TaskEntity?>> findById(int id, {bool includeDeleted = false}) async {
    try {
      final query = _database.select(_database.tasks)..where((row) => row.id.equals(id));
      if (!includeDeleted) query.where((row) => row.isDeleted.equals(false));
      final row = await query.getSingleOrNull();
      return Success(row == null ? null : _map(row));
    } on Object {
      return const FailureResult(DatabaseFailure(code: 'task_read_failed', safeMessage: 'The task could not be loaded.'));
    }
  }

  @override
  Future<Result<List<TaskEntity>>> list({int limit = 50, int offset = 0, bool includeDeleted = false}) async {
    if (limit < 1 || limit > 200 || offset < 0) {
      return const FailureResult(ValidationFailure(code: 'invalid_task_page', safeMessage: 'The requested task page is invalid.'));
    }
    try {
      final query = _database.select(_database.tasks);
      if (!includeDeleted) query.where((row) => row.isDeleted.equals(false));
      query
        ..orderBy([(row) => OrderingTerm.desc(row.updatedAt), (row) => OrderingTerm.asc(row.id)])
        ..limit(limit, offset: offset);
      return Success((await query.get()).map(_map).toList(growable: false));
    } on Object {
      return const FailureResult(DatabaseFailure(code: 'task_list_failed', safeMessage: 'Tasks could not be loaded.'));
    }
  }

  @override
  Future<Result<TaskEntity>> changeStatus(int id, TaskStatus status, {String? reason}) async {
    if (status == TaskStatus.deleted) {
      return softDelete(id, reason: reason);
    }
    try {
      return await _database.transaction(() async {
        final current = await _requireRaw(id, includeDeleted: status == TaskStatus.deleted);
        if (status == TaskStatus.completed) await _validateMandatoryChildren(id);
        if (current.status == TaskStatus.blocked && status == TaskStatus.completed) {
          throw StateError('A blocked task must be unblocked before completion.');
        }
        final now = _clock().toUtc();
        ...
        await (_database.update(_database.tasks)..where((row) => row.id.equals(id))).write(
          TasksCompanion(
            status: Value(status),
            preDeleteStatus: Value(deleting ? current.status : current.preDeleteStatus),
            progress: Value(status == TaskStatus.completed ? 100 : current.progress),
            completedAt: Value(status == TaskStatus.completed ? now.microsecondsSinceEpoch : null),
      ...
      return FailureResult(ValidationFailure(code: 'task_transition_rejected', safeMessage: error.message.toString()));
    } on Object {
      return const FailureResult(DatabaseFailure(code: 'task_transition_failed', safeMessage: 'The task state could not be changed safely.'));
    }
  }
  ...
  Future<Result<TaskEntity>> restore(int id, {String? reason}) async {
    try {
      ...
      if (current.parentTaskId != null) await _requireRaw(current.parentTaskId!, includeDeleted: false);
      final restored = current.preDeleteStatus == null || current.preDeleteStatus == TaskStatus.deleted
          ? TaskStatus.pending
          : current.preDeleteStatus!;
      return _restoreTo(id, restored, reason);
    } on StateError catch (error) {
      return FailureResult(ValidationFailure(code: 'task_restore_rejected', safeMessage: error.message.toString()));
    } on Object {
      return const FailureResult(DatabaseFailure(code: 'task_restore_failed', safeMessage: 'The task could not be restored safely.'));
    }
  }

  Future<Result<TaskEntity>> _restoreTo(int id, TaskStatus restored, String? reason) async {
    return _database.transaction(() async {
      ...
      await _recordState(id, TaskStatus.deleted, restored, now, reason ?? 'restored');
      await _refreshParentCount(current.parentTaskId, now);
      return Success(await _requireRow(id));
    });
  }
  ...
  Future<Result<TaskEntity>> duplicate(int id) async {
    ...
    if (source case FailureResult<TaskEntity?>(:final failure)) return FailureResult(failure);
    final task = (source as Success<TaskEntity?>).value;
    if (task == null) {
      return const FailureResult(ValidationFailure(code: 'task_not_found', safeMessage: 'The task does not exist.'));
    }
    return create(TaskDraft(
      title: task.title,
  ...
  @override
  Future<Result<void>> addDependency({required int taskId, required int dependsOnTaskId, required DependencyType type}) async {
    if (taskId == dependsOnTaskId) {
      return const FailureResult(ValidationFailure(code: 'dependency_self_cycle', safeMessage: 'A task cannot depend on itself.'));
    }
    try {
      return await _database.transaction(() async {
        ...
        await _database.into(_database.taskDependencies).insert(
      ...
      return FailureResult(ValidationFailure(code: 'dependency_rejected', safeMessage: error.message.toString()));
    } on Object {
      return const FailureResult(DatabaseFailure(code: 'dependency_save_failed', safeMessage: 'The dependency could not be saved safely.'));
    }
  }
  ...
  Future<void> _validateParent(int? parentId, {int? taskId}) async {
    ...
    if (parentId == taskId) throw StateError('A task cannot be its own parent.');
    var depth = 1;
    var current = await _requireRaw(parentId, includeDeleted: false);
    final visited = <int>{parentId};
    while (current.parentTaskId != null) {
      if (current.parentTaskId == taskId || !visited.add(current.parentTaskId!)) {
        throw StateError('The task hierarchy would contain a cycle.');
      }
      depth += 1;
      if (depth > 8) throw StateError('Task nesting cannot exceed eight levels.');
  ...
  Future<void> _validateMandatoryChildren(int id) async {
    final query = _database.select(_database.tasks)
      ..where((row) => row.parentTaskId.equals(id) & row.isMandatory.equals(true) & row.isDeleted.equals(false));
    final children = await query.get();
    if (children.any((child) => child.status != TaskStatus.completed)) {
      throw StateError('Complete all mandatory subtasks before completing this task.');
    }
  }
  ...
  Future<void> _refreshParentCount(int? parentId, DateTime now) async {
    ...
    final query = _database.selectOnly(_database.tasks)
      ..addColumns([count])
      ..where(_database.tasks.parentTaskId.equals(parentId) & _database.tasks.isDeleted.equals(false));
    final value = (await query.getSingle()).read(count) ?? 0;
    await (_database.update(_database.tasks)..where((row) => row.id.equals(parentId))).write(
      TasksCompanion(subtaskCount: Value(value), updatedAt: Value(now.microsecondsSinceEpoch)),
    );
  }
  ...
  Future<TaskEntity> _requireRow(int id, {bool includeDeleted = false}) async => _map(await _requireRaw(id, includeDeleted: includeDeleted));

  Future<void> _recordState(int taskId, TaskStatus? previous, TaskStatus next, DateTime at, String? reason) async {
  ...
  DateTime? _date(int? value) => value == null ? null : DateTime.fromMicrosecondsSinceEpoch(value, isUtc: true);
}