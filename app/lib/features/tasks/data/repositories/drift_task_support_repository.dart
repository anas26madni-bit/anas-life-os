...
final class DriftTaskSupportRepository implements TaskSupportRepository {
  ...
  @override
  Future<Result<int>> createCategory(
    String name, {
    String? description,
  }) async {
  ...
  Future<Result<int>> createSubcategory({
    ...
    try {
      ...
      final now = _now;
      final id = await _database.into(_database.subcategories).insert(
    ...
    try {
      return await _database.transaction(() async {
        ...
        await _database.into(_database.taskTags).insert(
          TaskTagsCompanion.insert(taskId: taskId, tagId: tagId, createdAt: now),
        ...
        await (_database.update(_database.tags)..where((row) => row.id.equals(tagId))).write(
      ...
      return const FailureResult(
        DatabaseFailure(code: 'tag_save_failed', safeMessage: 'The tag could not be saved safely.'),
      );
    }
  }
  ...
  Future<Result<int>> createChecklist({
    ...
    try {
      return await _database.transaction(() async {
        ...
        final now = _now;
        final id = await _database.into(_database.checklists).insert(
        ...
        await (_database.update(_database.tasks)..where((row) => row.id.equals(taskId))).write(
          TasksCompanion(
      ...
      return const FailureResult(
        DatabaseFailure(code: 'checklist_create_failed', safeMessage: 'The checklist could not be created safely.'),
      );
    }
  }
  ...
  Future<Result<int>> addChecklistItem({
    ...
    if (checklistId <= 0 || normalized == null) {
      return _invalid('invalid_checklist_item', 'Enter a valid checklist item.');
    }
    try {
      await _requireChecklist(checklistId);
      final now = _now;
      final id = await _database.into(_database.checklistItems).insert(
      ...
      return const FailureResult(
        DatabaseFailure(code: 'checklist_item_create_failed', safeMessage: 'The checklist item could not be created safely.'),
      );
    }
  }
  ...
  Future<Result<int>> addAttachment(AttachmentDraft draft) async {
    ...
    if (draft.taskId <= 0 || name == null || path == null || draft.fileSize < 0 || !_sha256.hasMatch(checksum)) {
      return _invalid('invalid_attachment', 'The attachment metadata is invalid.');
    }
    try {
      return await _database.transaction(() async {
        final task = await _requireTask(draft.taskId);
        final existing = await (_database.select(_database.attachments)
              ..where(
                (row) => row.taskId.equals(draft.taskId) & row.checksumSha256.equals(checksum) & row.isDeleted.equals(false),
              ))
            .getSingleOrNull();
        if (existing != null) {
          return Success(existing.id);
        }
        final canonical = await (_database.select(_database.attachments)
              ..where((row) => row.checksumSha256.equals(checksum) & row.isDeleted.equals(false)))
            .getSingleOrNull();
        final now = _now;
        final id = await _database.into(_database.attachments).insert(
        ...
        await (_database.update(_database.tasks)..where((row) => row.id.equals(draft.taskId))).write(
          TasksCompanion(
      ...
      return const FailureResult(
        DatabaseFailure(code: 'attachment_save_failed', safeMessage: 'The attachment metadata could not be saved safely.'),
      );
    }
  }
  ...
  Future<Result<int>> createRepeatRule(RepeatRuleDraft draft) async {
    ...
    final validEnd = switch (draft.endType) {
      RepeatEndType.never => draft.endAt == null && draft.occurrenceLimit == null,
      RepeatEndType.onDate => draft.endAt != null && draft.occurrenceLimit == null,
      RepeatEndType.afterOccurrences => draft.endAt == null && (draft.occurrenceLimit ?? 0) > 0,
    };
    if (name == null || timezone == null || draft.interval < 1 || draft.weekdayMask < 0 || draft.weekdayMask > 127 || (draft.dayOfMonth != null && (draft.dayOfMonth! < 1 || draft.dayOfMonth! > 31)) || (draft.monthOfYear != null && (draft.monthOfYear! < 1 || draft.monthOfYear! > 12)) || !validEnd) {
      return _invalid('invalid_repeat_rule', 'The repeat rule is invalid.');
    }
    try {
      final now = _now;
      final id = await _database.into(_database.repeatRules).insert(
      ...
      return const FailureResult(
        DatabaseFailure(code: 'repeat_rule_create_failed', safeMessage: 'The repeat rule could not be created safely.'),
      );
    }
  }
  ...
  Future<TaskRow> _requireTask(int id) async {
    final row = await (_database.select(_database.tasks)
        ...
        .getSingleOrNull();
    if (row == null) {
      throw StateError('The task does not exist.');
    }
    return row;
  }

  Future<ChecklistRow> _requireChecklist(int id) async {
    final row = await (_database.select(_database.checklists)
          ..where((row) => row.id.equals(id) & row.isDeleted.equals(false)))
        .getSingleOrNull();
    if (row == null) {
      throw StateError('The checklist does not exist.');
    }
    return row;
  }
  ...
  String? _requiredText(String value, int maxLength) {
    ...
    return normalized.isEmpty || normalized.runes.length > maxLength ? null : normalized;
  }
  ...
  FailureResult<T> _invalid<T>(String code, String message) => FailureResult(ValidationFailure(code: code, safeMessage: message));

  FailureResult<T> _rejected<T>(String code, StateError error) => FailureResult(ValidationFailure(code: code, safeMessage: error.message.toString()));
