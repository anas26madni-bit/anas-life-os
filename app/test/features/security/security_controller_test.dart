...
void main() {
  test(
    'configures, locks, unlocks, updates, changes and disables PIN',
    () async {
      ...
      expect(
        (await container.read(securityControllerProvider.future)).isLocked,
        isFalse,
      );
      ...
      notifier.lock();
      expect(
        container.read(securityControllerProvider).requireValue.isLocked,
        isTrue,
      );
      expect(container.read(authorizationGateProvider).isAuthorized, isFalse);

      platform.verifyResult = const UnlockResult(
        success: false,
        failedAttempts: 1,
      );
      ...
      await notifier.updatePreferences(
        repository.preferences.copyWith(
      ...
      expect(
        repository.audits,
        containsAll(['pin_configured', 'pin_changed', 'pin_disabled']),
      );
    },
  );
...
final class _SecurityRepository implements SecurityRepository {
  ...
  @override
  Future<SecurityPreferences> loadPreferences() async => preferences;
  @override
  Future<void> savePreferences(SecurityPreferences value) async {
    preferences = value;
  }
...
final class _SecurityPlatform implements SecurityPlatform {
  ...
  }) async => const UnlockResult(success: true);
  @override
  ...
  Future<void> setSecureWindow(bool enabled) async {
    secureWindow = enabled;
  }

  @override
  Future<UnlockResult> verifyPin(String pin) async => verifyResult;
}
