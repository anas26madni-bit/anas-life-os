...
void main() {
  test('persists approved security settings and redacted audit events', () async {
    ...
    expect((await database.select(database.appLockSessions).get()).single.failedAttempts, 2);
    expect((await database.select(database.securityAuditLog).get()).single.safeDetail, isEmpty);
  });
}
