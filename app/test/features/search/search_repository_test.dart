...
void main() {
  test(
    ...
    () async {
      ...
      final mixed =
          (await repository.search(const SearchQuery(text: 'budget بجٹ'))
      ...
      expect(
        (await repository.saved() as Success<List<SavedSearch>>).value,
        hasLength(1),
      );
      expect(
        (await repository.recent() as Success<List<RecentSearch>>).value,
        hasLength(2),
      );
  ...
  test('hidden and deleted source content never enters the index', () async {
    ...
    final allowed =
        (await repository.search(const SearchQuery(text: 'token'))
                as Success<List<SearchResultItem>>)
            .value;
  ...
  test(
    '100,000 mixed-language records search within 300 ms p95',
    () async {
      final database = createTestDatabase();
      addTearDown(database.close);
      final repository = DriftSearchRepository(
      ...
      for (var index = 0; index < 20; index++) {
        ...
        final result = await repository.search(
          const SearchQuery(text: 'needle خاص'),
        );
        stopwatch.stop();
