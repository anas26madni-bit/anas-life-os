...
void main() {
  test('persists reminder lifecycle and append-only idempotent history', () async {
    ...
    expect(
      (await repository.history() as Success<List<ReminderHistoryEntity>>).value,
      hasLength(1),
    );

    expect((await repository.softDelete(created.id) as Success<ReminderEntity>).value.isDeleted, isTrue);
    expect((await repository.restore(created.id) as Success<ReminderEntity>).value.isDeleted, isFalse);
  });
