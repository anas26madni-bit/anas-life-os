...
void main() {
  test('clones a standalone task and its subtask graph', () async {
    ...
    final source =
        (await tasks.create(const TaskDraft(title: 'Source'))
    ...
    final clone =
        (await composition.clone(source.id) as Success<TaskEntity>).value;
  ...
  test('moves a task subtree to a project', () async {
    ...
    final root =
        (await tasks.create(const TaskDraft(title: 'Root'))
                as Success<TaskEntity>)
            .value;
    final child =
        (await tasks.create(TaskDraft(title: 'Child', parentTaskId: root.id))
                as Success<TaskEntity>)
            .value;
  ...
  test('merge and split archive sources atomically', () async {
    ...
    final split = await composition.split(
      ...
      parts: const [
        TaskDraft(title: 'Part A'),
        TaskDraft(title: 'Part B'),
      ],
    );
    expect((split as Success<List<TaskEntity>>).value, hasLength(2));
    expect(
      (await tasks.findById(mergedTask.id) as Success<TaskEntity?>)
          .value!
          .status,
      TaskStatus.archived,
    );
