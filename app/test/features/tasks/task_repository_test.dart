...
void main() {
  test('creates, updates, paginates, deletes, and restores a task', () async {
    ...
    final repository = DriftTaskRepository(database, clock: () => DateTime.utc(2026, 7, 30));

    final createdResult = await repository.create(const TaskDraft(title: 'Plan sprint', priority: TaskPriority.high));
    ...
    expect((await repository.list() as Success<List<TaskEntity>>).value, hasLength(1));
    expect(await database.select(database.taskHistory).get(), hasLength(1));

    final updated = await repository.update(created.id, const TaskDraft(title: 'Plan Sprint 3', progress: 25));
    ...
    expect((await repository.list() as Success<List<TaskEntity>>).value, isEmpty);
  ...
  test('enforces mandatory subtasks and dependency cycles', () async {
    ...
    final parent = (await repository.create(const TaskDraft(title: 'Parent')) as Success<TaskEntity>).value;
    await repository.create(TaskDraft(title: 'Mandatory child', parentTaskId: parent.id, isMandatory: true));
    expect(await repository.changeStatus(parent.id, TaskStatus.completed), isA<FailureResult<TaskEntity>>());

    final first = (await repository.create(const TaskDraft(title: 'First')) as Success<TaskEntity>).value;
    final second = (await repository.create(const TaskDraft(title: 'Second')) as Success<TaskEntity>).value;
    expect(
